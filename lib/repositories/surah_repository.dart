...
class SurahRepository {
  ...
  Future<SurahDetailModel> getById(int id) async {
    try {
      // Prefer scraped files in assets/verses, fallback to legacy path
      String data;
      try {
        data = await rootBundle.loadString('assets/verses/surah-$id.json');
      } catch (_) {
        data = await rootBundle.loadString(
          'assets/data/surah-details/surah-$id.json',
        );
      }
